...
(defn announce-new-ledger!
  ...
  (let [{:keys [ledger-id t commit-address server tx-id]} ledger-created-event]
    ...
    (subscriptions/publicize-new-ledger subscriptions ledger-id commit-address)
    ::new-ledger))

(defn announce-new-commit!
  [subscriptions watcher transaction-commited-event]
  (let [{:keys [ledger-id t commit-address tx-id server]} transaction-commited-event]
    (log/info "New transaction completed for" ledger-id
              "tx-id: " tx-id "by server:" server)
    (watcher/deliver-commit watcher tx-id ledger-id t commit-address)
    (subscriptions/publicize-commit subscriptions ledger-id commit-address)
    ::new-commit))
