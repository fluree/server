...
(defprotocol TxGroup
  ...
  (-local-state [group])
  (-current-state [group] "Returns current state machine state")
  (-update-in [group ks f] "Updates current state by providing single-arity fn (as in Clojure's 'update-in' but without & args)")
  (-is-leader? [group])
...
(defn initialized-ledger-async
  ...
  (let [status  (util/without-nils {:status    :ready
                                    ...
                                    :index     index})
        command [:initialized-ledger txid network ledger-id status]]
    (-new-entry-async group command)))
